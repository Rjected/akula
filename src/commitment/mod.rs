...
impl HexPatriciaHashed {
    ...
    fn compute_cell_hash(&mut self, pos: Option<CellPosition>, depth: usize) -> H256 {
        ...
        let mut storage_root_hash = EMPTY_ROOT;
        if let Some((address, location)) = cell.spk {
            ...
            let h = hash_key(&spk, &mut cell.down_hashed_key, hashed_key_offset);
            ...
            if singleton {
                ...
                // let         storage_root_hash = hph.leaf_hash_with_key_val(nil, cell.downHashedKey[:64-hashedKeyOffset+1], rlp.RlpSerializableBytes(cell.Storage[:cell.StorageLen]), true);
                //         storage_root_hash = storage_root_hash[1..];
            } else {
                trace!(
...
fn leaf_hash_with_key_val(
    ...
    let buf = complete_leaf_hash(buf, kp, kl, compact_len, key, compact0, ni, val, singleton);
    // if err != nil {
    // 	return nil, err
    // }
    // return buf, nil
