...
impl TryFrom<usize> for EthMessageId {
    ...
    fn try_from(value: usize) -> Result<Self, Self::Error> {
        match value {
            0 => Ok(Self::Status),
            1 => Ok(Self::NewBlockHashes),
            2 => Ok(Self::Transactions),
            3 => Ok(Self::GetBlockHeaders),
            4 => Ok(Self::BlockHeaders),
            5 => Ok(Self::GetBlockBodies),
            6 => Ok(Self::BlockBodies),
            7 => Ok(Self::NewBlock),
            8 => Ok(Self::NewPooledTransactionHashes),
            9 => Ok(Self::GetPooledTransactions),
            10 => Ok(Self::PooledTransactions),
            13 => Ok(Self::GetNodeData),
            14 => Ok(Self::NodeData),
            15 => Ok(Self::GetReceipts),
            16 => Ok(Self::Receipts),
            id => Err(anyhow::format_err!(
                "EthMessageId does not contain the id {:?}",
                id
            )),
        }
    }
